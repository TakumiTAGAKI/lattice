...
def random_xy(pixel):
    ...
    s = int(num_dot/4)
    if (pixel == "int"):
        x1 = rng.integers(low=0, high=int(i/2), size=s)/(i/2) - 1
        x2 = rng.integers(low=int(i/2), high=i, size=3*s)/(i/2) - 1
        y1 = rng.integers(low=0, high=int(i/2), size=3*s)/(i/2) - 1
        y2 = rng.integers(low=int(i/2), high=i, size=s)/(i/2) - 1
    elif (pixel == 'float'):
        x1 = rng.random(s) * (-1)
        x2 = rng.random(3*s) * 1
        y1 = rng.random(3*s) * (-1)
